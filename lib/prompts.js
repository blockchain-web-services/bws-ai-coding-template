 ...
 * @param {Object} existingConfig - Optional existing configuration from .worktrees
 * @param {boolean} addAWS - Whether --add-aws flag was passed
 * @returns {Promise<Object>} Configuration object
 */
export async function askProjectInfo(projectRoot, existingConfig = null, addAWS = false) {
    ...
    // Use existing config values if available, otherwise use detected/default values
    const projectNameDefault = existingConfig?.projectName || defaultName || 'my-project';
    const githubUsernameDefault = existingConfig?.githubUsername || '';

    const questions = [
        {
            type: 'input',
            name: 'projectName',
            message: existingConfig ? `Project name (current: ${existingConfig.projectName}):` : 'Project name:',
        ...
        },
        {
            type: 'input',
            name: 'githubUsername',
            message: existingConfig ? `GitHub username or organization (current: ${existingConfig.githubUsername}):` : 'GitHub username or organization:',
    ...
    return {
        ...
        useAWS: addAWS, // Set based on --add-aws flag
        repositoryName: detectedRepoName || existingConfig?.repositoryName || answers.projectName.trim()
    };
}
...
export async function confirmInstallation(config, conflicts = []) {
    ...
    console.log(chalk.green('  ✓'), 'test/', chalk.gray('- Basic test setup with Playwright'));
    console.log(chalk.green('  ✓'), '.github/workflows/test.yml', chalk.gray('- GitHub Actions test workflow'));

    if (config.useAWS) {
        console.log(chalk.green('  ✓'), 'test/ (AWS)', chalk.gray('- LocalStack test infrastructure'));
