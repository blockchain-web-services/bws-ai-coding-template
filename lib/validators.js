...
export async function validateNoConflicts(projectRoot, useAWS) {
    ...
    // Check for test or tests folder (basic tests are always installed)
    ...
    if (await fs.pathExists(testDir)) {
        conflicts.push({
            path: 'test/',
            message: 'Test folder already exists. Files will be merged/updated.',
            severity: 'warning'
        });
    } else if (await fs.pathExists(testsDir)) {
        conflicts.push({
            path: 'tests/',
            message: 'Tests folder exists. Basic tests will be installed to test/ instead.',
            severity: 'warning'
        });
    }

    // AWS-specific conflicts
    if (useAWS) {
        const deployDir = path.join(projectRoot, '.deploy');
        if (await fs.pathExists(deployDir)) {
            conflicts.push({
                path: '.deploy/',
                message: 'AWS deployment folder already exists. Remove it or run without --add-aws flag.',
                severity: 'error'
            });
        }
    }
 ...
 */
export async function createDefaultPackageJson(projectRoot) {
    const pkgPath = path.join(projectRoot, 'package.json');

    // Get repository name from git remote or use directory name
    let repoName = await detectRepositoryName(projectRoot);
    if (!repoName) {
    ...
    const defaultPackage = {
        ...
        keywords: [],
        author: "blockchain-web-services",
        license: "MIT",
        repository: {
            type: "git",
            url: `git+https://github.com/blockchain-web-services/${repoName}.git`
        },
        bugs: {
            url: `https://github.com/blockchain-web-services/${repoName}/issues`
        },
        homepage: `https://github.com/blockchain-web-services/${repoName}#readme`
    };

    await fs.writeJson(pkgPath, defaultPackage, { spaces: 2 });
}
